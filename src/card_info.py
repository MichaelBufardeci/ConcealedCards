...
class CardInfo:
    ...
    def __init__(
        ...
        if self.set_code == "ENERGY":
            ...
            for energy_symbol, energy_name in energy_symbols.items():
                self.name = re.sub( f"{{{energy_symbol}}}", f"{energy_name}", self.name)
    ...
    def lookup_from_database(self):
        ...
        query_root = "SELECT regMark, type, isStandardLegal, isExpandedLegal FROM cards WHERE "
